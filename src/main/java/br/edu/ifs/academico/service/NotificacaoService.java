...
public class NotificacaoService {
        ...
        @Scheduled(cron = "${notificacao.cleanup.cron:0 0 2 * * *}")
        @Transactional
        public void removerNotificacoesExpiradas() {
                LocalDateTime agora = LocalDateTime.now();
                List<Notificacao> expiradas = notificacaoRepository.findByDataExpiracaoBefore(agora);

                if (expiradas.isEmpty()) {
                        return;
                }

                List<Long> idsExpiradas = expiradas.stream()
                                .map(Notificacao::getId)
                                .toList();

                notificacaoUsuarioRepository.deleteByNotificacao_IdIn(idsExpiradas);
                notificacaoRepository.deleteAllByIdInBatch(idsExpiradas);

                log.info("Notificações expiradas removidas: {}", idsExpiradas.size());
        }
}
